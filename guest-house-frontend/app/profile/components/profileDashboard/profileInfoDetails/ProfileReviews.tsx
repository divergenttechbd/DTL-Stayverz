...
import { getReviewsByMe, getReviewsForMe } from '~/queries/client/reviews'
...
const ProfileReviews: FC<IProfileReviewsProps> = ({ latestReviews, firstName }) => {
  ...
  const { data: reviewsForMe, isFetching } = useQuery({
    queryKey: ['reviewsForMe'],
    queryFn: getReviewsForMe,
  ...
  return (
    <>
      <hr className='mb-5 w-full border-t border-grayBorder' />

      <div className='mb-5 relative'>
        <h3 className='text-2xl font-medium mb-8'>{firstName}&apos;s reviews</h3>
      ...
      <button
        className='text-black underline font-medium pb-2 mt-3 pt-2 rounded-lg text-base hover:bg-gray-50 mb-5'
